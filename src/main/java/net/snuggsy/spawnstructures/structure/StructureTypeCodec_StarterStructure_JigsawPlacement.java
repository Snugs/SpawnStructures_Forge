...
import static net.snuggsy.spawnstructures.functions.GenerationFunctions.setStartPool;

public class StructureTypeCodec_StarterStructure_JigsawPlacement {
    ...
    public static Optional<Structure.GenerationStub> addPieces(Structure.GenerationContext pContext, Holder<StructureTemplatePool> pStartPool, Optional<ResourceLocation> pStartJigsawName, int pMaxDepth, BlockPos pPos, boolean pUseExpansionHack, Optional<Heightmap.Types> pProjectStartToHeightmap, int pMaxDistanceFromCenter) {
        newLog("Starter Structure attempted placement at: " + pPos + " !!!");
        ...
        if (structurepoolelement == EmptyPoolElement.INSTANCE) {
            ...
            if (pStartJigsawName.isPresent()) {
                ...
                if (optional.isEmpty()) {
                    LOGGER.error("[Spawn Structures] No starting jigsaw {} found in start pool {}", resourcelocation, pStartPool.unwrapKey().map((p_248484_) -> {
    ...
    private static void addPieces(RandomState pRandomState, int pMaxDepth, boolean pUseExpansionHack, ChunkGenerator pChunkGenerator, StructureTemplateManager pStructureTemplateManager, LevelHeightAccessor pLevel, RandomSource pRandom, Registry<StructureTemplatePool> pPools, PoolElementStructurePiece p_227219_, List<PoolElementStructurePiece> pPieces, VoxelShape p_227221_) {
        StructureTypeCodec_StarterStructure_JigsawPlacement.Placer jigsawplacement$placer = new StructureTypeCodec_StarterStructure_JigsawPlacement.Placer(pPools, pMaxDepth, pChunkGenerator, pStructureTemplateManager, pPieces, pRandom);
        jigsawplacement$placer.placing.addLast(new StructureTypeCodec_StarterStructure_JigsawPlacement.PieceState(p_227219_, new MutableObject<>(p_227221_), 0));

        while(!jigsawplacement$placer.placing.isEmpty()) {
            StructureTypeCodec_StarterStructure_JigsawPlacement.PieceState jigsawplacement$piecestate = jigsawplacement$placer.placing.removeFirst();
    ...
    static final class Placer {
        ...
        final Deque<StructureTypeCodec_StarterStructure_JigsawPlacement.PieceState> placing = Queues.newArrayDeque();
                                        ...
                                        if (JigsawBlock.canAttach(structuretemplate$structureblockinfo, structuretemplate$structureblockinfo1)) {
                                            ...
                                            if (!Shapes.joinIsNotEmpty(mutableobject1.getValue(), Shapes.create(AABB.of(boundingbox3).deflate(0.25D)), BooleanOp.ONLY_SECOND)) {
                                                ...
                                                if (pDepth + 1 <= this.maxDepth) {
                                                    this.placing.addLast(new StructureTypeCodec_StarterStructure_JigsawPlacement.PieceState(poolelementstructurepiece, mutableobject1, pDepth + 1));
                                                }
                                                continue label129;
                                            }
                                        }
